...
var analyzeLocalHeuristics = (inputUrl) => {
  ...
  let classifierVerdict = "LEGITIMATE";
  let confidence = "99.8%";
  let verdictText = "";
  ...
  const domainAuthorityExp = isDirectSafe ? `The domain is a global top-tier site with massive historical traffic and a positive reputation.` : `This domain is not listed in global top-10000 high-reputation domains, suggesting smaller traffic volume.`;
  const sslSecurityValue = isSslInsecure ? "Insecure HTTP" : "Valid";
  const sslSecurityExp = isSslInsecure ? "Uses unencrypted HTTP protocols. Any data entered can be intercepted in transit." : `Uses a valid, secure SSL/TLS certificate to encrypt transmission protocols.`;
  const heuristicPatternValue = hostKeywordsMatched.length + pathKeywordsMatched.length > 0 ? "Positive" : "Negative";
  const heuristicPatternExp = hostKeywordsMatched.length + pathKeywordsMatched.length > 0 ? `Detected suspicious keywords: ${[...hostKeywordsMatched, ...pathKeywordsMatched].join(", ")} in the URL structure.` : "No suspicious keywords like 'login', 'secure', or 'update' detected in the path or subdomain.";
  const subdomainDepthValue = String(subdomainCount);
  const subdomainDepthExp = subdomainCount > 1 ? `Uses excessive subdomains (${subdomainCount}) to potentially mask the true destination domain.` : "The URL does not use excessive subdomains to obfuscate the destination.";
  return {
    score,
    badge,
    badgeColor,
...
Analyze the URL's structure, domain reputation, potential brand impersonation, SSL indicators, and heuristic pattern matching.`,
      ...
      const localAnalysis2 = analyzeLocalHeuristics(url);
      localAnalysis2.fallbackReason = isUnavailable ? "The live Gemini AI model is currently experiencing extremely high global demand. Showing advanced local heuristic scan instead." : "Seamless fallback to advanced local heuristic scan (Gemini connection offline or pending configuration).";
      return res.json(localAnalysis2);
    }
  }
